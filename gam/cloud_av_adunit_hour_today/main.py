"""
cloud_av_adunit_hour_today — Snapshot intradiário de performance + regras de preço ActiveView.

Captura 2 fontes por site:
1. Performance (from-gam): eCPM, impressions, PMR acumulados do dia (today)
2. Regras de preço (rules): floor price atual, state (AUTO/MANUAL)

Grava em 2 tabelas BigQuery (WRITE_APPEND):
- test.cloud_av_adunit_hour (performance)
- test.cloud_av_pricingrules_hour (regras)

Roda 5x/dia (05:30, 09:30, 13:30, 17:30, 21:30 BRT).
"""
...
async def run_async():
    ...
    print(f"=== AV Snapshot TODAY — {captured_at} ===")
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [
            fetch_site(session, s["network_id"], s["site"], date_str)
            for s in GAM_SITES
        ]
