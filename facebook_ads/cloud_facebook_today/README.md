# Facebook Ads Today - GitHub Actions

Este projeto coleta dados de performance de campanhas do Facebook de **HOJE** e os envia para o BigQuery usando GitHub Actions.

## 🎯 Objetivo

Coletar métricas diárias de campanhas do Facebook de **HOJE** e armazenar no BigQuery com sobrescrita completa (WRITE_TRUNCATE).

## 📅 Período de Coleta

- **Data coletada:** HOJE (date_preset: "today")
- **Exemplo:** Se hoje é 07/01/2026, coleta dados de 07/01/2026
- **Atualização:** A cada hora no minuto 58 (00:58, 01:58, 02:58, etc. - horário UTC)
...
**BigQuery:**
- **Tabela:** `data-v1-423414.test.cloud_facebook_today_ca`
...
```
facebook_ads/cloud_facebook_today/
...
- **Agendado**: A cada hora no minuto 58 (em UTC)
  - Exemplo: 00:58 UTC, 01:58 UTC, 02:58 UTC, etc.
...
3. Execute:
```bash
cd facebook_ads/cloud_facebook_today
...
📊 Linhas finais: 2000
☁️ Enviando 2000 registros para data-v1-423414.test.cloud_facebook_today_ca...
✅ Adicionados 2000 registros para data-v1-423414.test.cloud_facebook_today_ca
```

## ⚠️ Notas Importantes

1. **Data de coleta:** O script coleta dados de HOJE (date_preset: "today")
2. **Timezone:** Usa timezone de São Paulo (America/Sao_Paulo) para `imported_at`
3. **Modo de escrita:** TRUNCATE (sobrescreve completamente os dados a cada execução)
4. **Frequência:** Executa a cada hora para manter os dados atualizados
...
```sql
CREATE TABLE `data-v1-423414.test.cloud_facebook_today_ca` (
  account_name STRING,
  account_id STRING,
  campaign_id STRING,
...
2. O workflow está configurado em:
   - `.github/workflows/cloud_facebook_today.yml`
